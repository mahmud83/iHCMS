...
/**
 * This is the model class for table "karyawan_pendidikan".
 *
 * The followings are the available columns in table 'karyawan_pendidikan':
 * @property integer $id
 ...
 */
class KaryawanPendidikan extends CActiveRecord
{
	/**
	 * Returns the static model of the specified AR class.
	 * @param string $className active record class name.
	 * @return KaryawanPendidikan the static model class
	 */
	...
	public function rules()
	{
		...
		return array(
			array('karyawan_id', 'required'),
			array('karyawan_id', 'numerical', 'integerOnly'=>true),
			array('jenis, institusi', 'length', 'max'=>100),
			array('major', 'length', 'max'=>45),
			array('nilai', 'length', 'max'=>25),
			array('tgl_masuk, tgl_keluar', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, karyawan_id, jenis, institusi, major, nilai, tgl_masuk, tgl_keluar', 'safe', 'on'=>'search'),
		);
	}
